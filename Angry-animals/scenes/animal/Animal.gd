...
func _unhandled_input(event: InputEvent) -> void:
	if _state == AnimalState.Drag and event.is_action_released("drag"):
		#change_state(AnimalState.Release) detecting the input but defering
		#the call so the engine can calculate everything correctly
...
#endregion
#region state
#update state is going to be called on every frame
